...
class LumbarPage extends StatefulWidget {
  const LumbarPage({super.key});

  @override
  State<LumbarPage> createState() => _LumbarPageState();
}

class _LumbarPageState extends State<LumbarPage> {
  double overlayBaseSize = 300;
  bool isAnalyzingAp = false;
  bool isAnalyzingLa = false;

  Uint8List? lumbarApBytes;
  Uint8List? lumbarLaBytes;
  ...
  Future<void> _prepareImages() async {
    ...
    final dynamic lumbarApData = cropState.getCropImage(
        ViewType.ap, 'lumbar');
    final dynamic lumbarLaData = cropState.getCropImage(
        ViewType.la, 'lumbar');

    // AP 이미지 처리
    if (lumbarApData != null) {
      try {
        print('🔍 AP 데이터 타입: ${lumbarApData.runtimeType}');

        // 데이터가 문자열처럼 보이지만 실제로는 리스트 형태일 수 있음
        if (lumbarApData is String && lumbarApData.startsWith('[') &&
            lumbarApData.contains(',')) {
          try {
            // 문자열을 파싱하여 int 리스트로 변환
            final String cleanString = lumbarApData
                .replaceAll('[', '')
                .replaceAll(']', '')
                .replaceAll(' ', '');
            ...
            lumbarApBytes = Uint8List.fromList(intList);
        ...
        // 일반 리스트인 경우
        else if (lumbarApData is List) {
          List<int> intList = [];
          for (var item in lumbarApData) {
            if (item is int) {
              intList.add(item);
        ...
        // 일반 base64 문자열인 경우
        else if (lumbarApData is String) {
          try {
            lumbarApBytes = base64Decode(lumbarApData);
            ...
            try {
              ...
              for (int i = 0; i < lumbarApData.length; i++) {
                bytes.add(lumbarApData.codeUnitAt(i));
              }
              lumbarApBytes = Uint8List.fromList(bytes);
        ...
        // 이미지 크기 정보 계산
        if (lumbarApBytes != null) {
          try {
            final apImage = await decodeImageFromList(lumbarApBytes!);
            setState(() {
    ...
    // LA 이미지도 동일한 방식으로 처리
    if (lumbarLaData != null) {
      try {
        print('🔍 LA 데이터 타입: ${lumbarLaData.runtimeType}');

        // 데이터가 문자열처럼 보이지만 실제로는 리스트 형태일 수 있음
        if (lumbarLaData is String && lumbarLaData.startsWith('[') &&
            lumbarLaData.contains(',')) {
          try {
            // 문자열을 파싱하여 int 리스트로 변환
            final String cleanString = lumbarLaData
                .replaceAll('[', '')
                .replaceAll(']', '')
                .replaceAll(' ', '');
            ...
            lumbarLaBytes = Uint8List.fromList(intList);
        ...
        // 일반 리스트인 경우
        else if (lumbarLaData is List) {
          List<int> intList = [];
          for (var item in lumbarLaData) {
            if (item is int) {
              intList.add(item);
            } else if (item is String) {
              try {
        ...
        // 일반 base64 문자열인 경우
        else if (lumbarLaData is String) {
          try {
            lumbarLaBytes = base64Decode(lumbarLaData);
            ...
            try {
              // 직접 바이트 배열 생성 시도
              final List<int> bytes = [];
              for (int i = 0; i < lumbarLaData.length; i++) {
                bytes.add(lumbarLaData.codeUnitAt(i));
              }
              lumbarLaBytes = Uint8List.fromList(bytes);
        ...
        // 이미지 크기 정보 계산
        if (lumbarLaBytes != null) {
          try {
            final laImage = await decodeImageFromList(lumbarLaBytes!);
            setState(() {
  ...
  Widget build(BuildContext context) {
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      lumbarApBytes != null
                          ? Image.memory(
                          lumbarApBytes!, height: 150, fit: BoxFit.contain)
                          : const Text(
                          '이미지 없음', style: TextStyle(color: Colors.white70)),
                      const SizedBox(height: 12),
                      ElevatedButton.icon(
                        onPressed: lumbarApBytes == null
                            ? null
                            : () async {
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      lumbarLaBytes != null
                          ? Image.memory(
                          lumbarLaBytes!, height: 150, fit: BoxFit.contain)
                          : const Text(
                          '이미지 없음', style: TextStyle(color: Colors.white70)),
                      const SizedBox(height: 12),
                      ElevatedButton.icon(
                        onPressed: lumbarLaBytes == null
                            ? null
                            : () async {
                          await _analyzeKeypointsAndShowDialog(
                              context,
    ...
    Widget scrollableResultSection = keypointsState
        .getOverlayedKeypoints('lumbar', 'LA')
        ?.isNotEmpty ?? false
        ? Container(
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.black,
        title: const Text('요추 분석', style: TextStyle(color: Colors.white)),
        centerTitle: true,
        leading: IconButton(
